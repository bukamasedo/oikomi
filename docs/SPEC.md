...
#### 4.2.2 AI コーチング（v1.0 では 3つに絞る）

| 提案タイプ | トリガー | 例 |
|---|---|---|
| ディロード推奨 | HRV が直近2週平均より15%以上低下 | "今日は HRV が落ちています。前回比80%の重量で組みましょう" |
| PR 予測 | 直近5回の伸び率から推定 | "次回ベンチで PR 更新の可能性あり（推定 87.5kg）" |
...
| レイヤー | 技術 | 採用理由 |
|---|---|---|
| UI | **SwiftUI** | 宣言的、watchOS / iOS / iPadOS で共通化 |
...
| 認証 | **Sign in with Apple** | メアド管理不要、プライバシー訴求 |
...
### 7.3 オンボーディング（5ステップ・各スキップ可）

1. **ウェルカム** — 3つの体験価値（手首完結 / 賢い / 日本語ネイティブ）を簡潔に紹介
2. **Sign in with Apple** — アカウント作成（任意、ローカル利用も可）
3. **HealthKit 権限** — 読み書き範囲を説明して許可リクエスト
4. **Apple Watch ペアリング確認** — 未所持の場合はスキップ
5. **ルーティン作成** — プリセットから選ぶ or 後で
...
- **iPad**: 3カラムレイアウト（履歴 / 詳細 / 分析グラフ）
- **Mac (Catalyst)**: iPad UI を流用、サイドバーナビゲーション

### 7.7 ナビゲーション構造

- **iPhone**: タブバー（5タブ）
- **Apple Watch**: フラットなナビゲーションスタック
- **iPad / Mac**: `NavigationSplitView`（v2.0）
...
| v2.0 | 拡張 | iPad 詳細分析 / Vision Pro フォーム確認 / 英語UI |

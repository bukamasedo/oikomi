...
- 今日のレディネス/追い込み推奨は Free。トレンド/長期履歴/PR予測/ボリューム警告など「深さ」タップ時にソフトウォール（「Pro 機能です」表示）
...
**Tier 2: Apple 特化（楔は Free・深さ/分析は Pro）**

- HealthKit 読み込み（HRV / 睡眠 / 安静時心拍）→ **Free**（楔の計算に必須）
- 今日のレディネス判定＋追い込み/ディロード推奨 → **Free**（＝楔の1シグナル、全ユーザーに体感させる）
- レディネス/HRV トレンド・長期履歴・ACWR、線形回帰 PR予測、部位別ボリューム警告 → **Pro**（深さ・自動化・縦断）
- 1RM 自動計算・推定強度（%1RM）
- 高度な分析グラフ → **Pro**
...
| プラン | 価格 | 内容 |
|---|---|---|
| **Free** | 無料・広告なし | 基本記録機能、履歴閲覧無制限、Apple Watch記録、HealthKit書き込み、**HealthKit 読み込み（HRV/睡眠/安静時心拍）**、**今日のレディネス判定＋追い込み/ディロード推奨（＝楔の1シグナル）**、ルーティン5つまで、カスタム種目5つまで、Live Activity / Dynamic Island |
| **Pro 月額** | ¥780/月 | Pro 機能アンロック |
| **Pro 年額** | ¥4,500/年 | Pro 機能アンロック（実質 ¥375/月、月額比 52%オフ） |

> 年額は 2026-06-15 の日本 WTP リサーチ（Task w1at2pfl1, 20/25 確証）で **¥5,800→¥4,500** に改定。最近接競合 Athlytic（JP ¥800/mo・¥3,400/yr）・Gentler Streak（¥1,250/mo・~¥5,000/yr）に対し、月額 ¥780 は妥当だが、旧年額 ¥5,800 は全比較対象（Athlytic ¥3,400 / Hevy ¥2,600 / あすけん ¥3,600）を超えて突出していたため。日本の ARPU/転換率/弾力性は未測定（競合価格アンカーのみ。確定には Van Westendorp 等の WTP テストが必要）。

### Pro 限定機能（深さ・自動化・縦断・同期）

- レディネスの**深さ**：HRV/レディネスのトレンドグラフ・長期履歴・急性:慢性負荷比（ACWR）
- AI コーチングの**深さ**：線形回帰 PR 予測・部位別ボリューム警告（※今日のレディネス判定とディロード/追い込み推奨そのものは Free）
...
### 課金境界の原則（split 型：楔は無料・深さは有料）

**Free = 楔を"体感"（今日どれだけ追い込むかの1シグナル）／ Pro = 楔を"運用"（深さ・自動化・縦断・同期）。**

- 差別化要因（レディネス駆動コーチング）を壁の内側に丸ごと隠さない。無料ユーザーが初日に楔を体感できることが、再定位（§2）の「使う理由」を伝える前提。
- HealthKit 読み込みと「今日のレディネス＋追い込み/ディロード推奨」は Free。トレンド/長期履歴/ACWR/PR予測/ボリューム警告/高度分析は Pro。
- Live Activity / Dynamic Island も Free（一番尖った体験を全ユーザーに）。
- 任意で、全 Pro 深さ機能の **期間限定トライアル**（習慣化させてから課金、Fitbod 型）を併用。

> 根拠（2026-06-15 deep-research、97エージェント/15ソース/25主張を3票検証）: 最も近い Apple Watch 回復系競合 **Athlytic / Gentler Streak は両方ともレディネス/回復スコアを無料で見せ、高度 HRV 分析・トレンド・長期履歴・パーソナル洞察を有料**にしている（free-to-feel / pay-for-depth、3-0 で確証）。RevenueCat の tier 設計ガイドも「差別化要因を壁にして無料層を"片輪のデモ"にする」のを最大の失敗と明記。ハードペイウォールの転換5倍（10.7% vs 2.1%）は生存バイアスで1年継続率はほぼ同じ＝楔を隠す根拠にならない（課金者 LTV は +21% ＝深さに課金壁を置く根拠にはなる）。なお「コアを気前よく無料化すれば転換・売上が伸びる」という強い主張は検証で反証（0-3）されたため、**"全部無料にして他で稼ぐ"は採らない**。未回答：日本固有 WTP。

### トライアル・割引戦略

- 全プランで **14日間無料トライアル**（クレジットカード登録必須）
- ローンチキャンペーン：先着1,000名 or 30日間 年額50%オフ（¥2,250）
- 年に2回程度、新規ユーザー向け年額50%オフキャンペーン

### 想定 ARPU

- ¥3,500〜4,500/ユーザー/年（年額・月額・無料の比率を加味。2026-06-15 の年額 ¥4,500 改定を反映）

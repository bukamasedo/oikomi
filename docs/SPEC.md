...
### 地域戦略
日本市場を主軸にローンチ。UI ローカライズは **日本語 + 英語の2言語**を v0.x で前倒し実装済み（種目名 873 種・コーチング文言・UI 文字列・HealthKit 許諾文を英訳）。英語圏への本格的なグローバル展開（ストア最適化・マーケ・追加言語）は v2.0 以降に検討。
...
| v2.0 | 拡張 | Vision Pro フォーム確認 / 英語圏グローバル展開（ストア最適化・追加言語） |

> 注: 英語 UI ローカライズ自体は v0.x で前倒し実装済み（§2 地域戦略）。v2.0 は英語圏への本格展開フェーズを指す。

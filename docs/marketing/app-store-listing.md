...
匿名識別子と表示名のみを取得します。メールアドレスは取得しません。
サインインしなくても全機能を利用できます。
...
```
筋トレ,ワークアウト,記録,AppleWatch,HRV,睡眠,コーチング,ジム,フィットネス,1RM,ルーティン,ベンチプレス
```
...
```
Sign in with Apple は任意機能のため、レビュー用デモアカウントは不要。
オンボーディングの「今はスキップ」で全機能をテスト可能。
HealthKit 権限は同様に「後で設定」でスキップ可能。
```
...
6. Sign in with Apple (任意)
   - オンボーディング 2 番目 / 設定タブのアカウントセクション
   - スキップ可能、全機能利用可能

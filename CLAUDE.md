...
- **App Store「最新情報」(What's New)** は `docs/marketing/app-store-listing.md` の「What's New」フォーマット規約に従い ja/en 対で作成・追記する（冒頭1行 → `▼ 新機能 / 改善 / 不具合修正` → 末尾の感謝、内部向け変更は載せない）
...
**Free**: 基本記録・履歴無制限・Watch 記録・HealthKit 書き込み・**HealthKit 読み取り（今日の HRV/睡眠/安静時心拍）**・**今日のレディネス判定＋追い込み/ディロード推奨（＝楔の1シグナル）**・ルーティン 5つ・カスタム種目 5つ・Live Activity / Dynamic Island
**Pro**: レディネス/HRV の**トレンド・長期履歴・ACWR**・AI コーチングの**深さ（PR予測・部位別ボリューム警告）**・高度な分析・ルーティン無制限・iCloud 同期・Family Sharing・データエクスポート

> 課金境界は split 型「楔は無料・深さは有料」（2026-06-15、SPEC §10「課金境界の原則」）。コード（ProGate）は未追従。
...
- ✅ Pro 機能ゲート（ルーティン 5 / カスタム種目 5 / HealthKit 読 / AI コーチング / iCloud 同期）— Live Activity は v0.x で Free 開放。⚠️ **2026-06-15 に課金境界を split 型へ再設計（SPEC §10）。HealthKit 今日読込と今日のレディネス/ディロード推奨を Free 化、深さ（PR予測/ボリューム警告/トレンド/長期履歴）を Pro 化する変更は ProGate に未実装**

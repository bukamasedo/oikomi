...
- ❌ Sign in with Apple（オンボーディング 2 番目、後回し可能）
- ❌ StoreKit 2 統合（v1.0 ローンチ前に予定。UI は `ProUpgradeSheet` で準備済み）
- ❌ HKWorkoutSession 並行起動 / Workout Buddy 共存（Tier 2.4、Watch 側で要実装）
- ❌ iCloud 同期（cloudKitDatabase: .none で待機中、Pro 機能としてローンチ前に有効化）
- ❌ 生理周期連携（v1.1）
- ❌ Family Sharing（v1.1）
...
- 39 ユニットテスト全成功（OneRepMax 6 / Repositories 7 / Routine 7 / Analytics 15 / PersonalRecord 4）
- 実行: `swift test --package-path Packages/OikomiKit`

### ビルド対応プラットフォーム

- ✅ iOS 26+ シミュレータ
- ✅ macOS 26 ネイティブ（プレースホルダ UI、本格化は v2.0）
- 🟡 watchOS 26 デバイスビルド（シミュレータコンポーネントは別途インストール要）
